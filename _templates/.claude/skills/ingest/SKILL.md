---
name: ingest
description: 将 raw/ 目录下的原始资料编译到 wiki/ 中。支持 /ingest（扫描所有未归档文件）、/ingest <path>（处理指定文件）或 /ingest <url>（抓取网页后处理）。当用户提到"摄取"、"导入"、"收入"资料时触发。绝对忽略 raw/09-archive/ 目录。
user-invocable: true
---
...
3. **生成本地文件名**：从 URL 提取域名和页面标题，生成 kebab-case 文件名
   - 格式：`{域名}-{页面slug}.md`
   - 示例：`https://example.com/article/transformer-explained` → `example-com-transformer-explained.md`
...
> **注意**：URL 摄入模式下，步骤 7（归档）仍然执行，源文件移动到 `raw/09-archive/`。
> 如果抓取失败（网络错误、页面不存在），向用户报告失败原因，不创建任何文件。
...
> 当用户通过 `/ingest <路径>` 处理单个文件时，默认启用讨论模式。
> 当用户通过 `/ingest` 批量扫描时，默认跳过讨论，直接编译（可在 SKILL 中配置 `discussion_mode: false`）。

在讨论模式下：
1. 向用户展示提取的核心主旨、实体列表、概念列表
2. 询问用户是否有补充或修正：
   > 我从这篇资料中提取了以下关键信息：
   > - **主旨**：...
   > - **实体**：[[A]], [[B]], [[C]]
   > - **概念**：[[X]], [[Y]]
   >
   > 是否需要调整？比如：重点关注的方面、需要忽略的内容、应当强调的角度？
3. 根据用户反馈调整后续编译
4. 用户确认后继续步骤 4
...
当 `/ingest` 扫描处理了多个文件时，全部处理完毕后输出汇总报告：

```markdown
## Ingest 摘要报告 — YYYY-MM-DD
...
### 建议后续动作
1. 核实 [[PageA]] 中的冲突项
2. [[PageB]] 为新创建的高频概念，建议补充更多来源
3. 考虑执行 `/lint` 检查知识库整体健康度
```
